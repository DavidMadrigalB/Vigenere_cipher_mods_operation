'''
    ...
    This is an example or basic implementation for vigenere mod CBC
'''
import os
import random
...
class vigenere_CBC:
    '''
    A class used from basic vigenere cipher CBC mod
    ...
    cifrar_cbc(blocks, llave, iv)
        Encipher using vigenere mod CBC
    cifrar_bloque_cbc(texto, llave, iv)
        Encipher using vigenere CBC mod, block cipher
    decifrar_cbc(ciphertext, llave, iv)
        Decipher using vigenere CBC mod
    decifrar_bloque_cbc(ciphertext, llave, iv)
    ...
    def cifrar_cbc(self, bloques, llave, iv):
        '''
        ...
        ciphertext = []
        #First block
        bloque_ciphertext = self.cifrar_bloque_cbc(bloques[0], llave, iv)
        ciphertext.append(bloque_ciphertext)
        
        #Follow blocks
        for i in range(1, len(bloques)):
            bloque_ciphertext = self.cifrar_bloque_cbc(bloques[i], llave, ciphertext[i-1])
            ciphertext.append(bloque_ciphertext)
            
        return ciphertext

    def cifrar_bloque_cbc(self, bloque, llave, iv):
        '''
    ...
    def decifrar_cbc(self, bloques, llave, iv):
        '''
        Decipher using vigenere CBC mod
        ...
        plaintext = []
        #First block
        bloque_ciphertext = self.decifrar_bloque_cbc(bloques[0], llave, iv)
        plaintext.append(bloque_ciphertext)
        #Follow blocks
        for i in range(1, len(bloques)):
            bloque_plaintext = self.decifrar_bloque_cbc(bloques[i], llave, bloques[i-1])
            plaintext.append(bloque_plaintext)
            
        return plaintext

    def decifrar_bloque_cbc(self, bloque, llave, iv):
        '''
